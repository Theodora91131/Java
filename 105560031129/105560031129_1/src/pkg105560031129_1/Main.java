...
class Main {
    public static void main(String[] args) throws Exception{
        ...
        //-------------------------------------------
       data.forEach(s -> {
            if(s.getHolidayPrice() >= s.getregularPrice()*1.5){
                String str = s.getId() + "," + s. getName() + "," + s.getTel()+ "," + s.getFax()+ "," + s.getAddress()+ "," + s.getLat()+ "," + s.getLng()+ "," + s.getregularPrice()+ "," + s.getHolidayPrice()+ "," + s.getRoom(); 
                output.add(str);              
            }   
        });
